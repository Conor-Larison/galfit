...
#
# Conor Larison last modified 10/6/2021
#
...
sigma = '/home/conor/Downloads/q2343_f140w_sig.fits'
...
for filename in fits_list:
	...
	obj_init = second[0:6] #hard-coded. Will have to be changed based on names
	...
	if comp == 1:
		...
		#These regions are so that all of the galaxies can be fit into the same model if close together

		


	effre = 2 #most Ly-alpha emitters at this redshift tend to have this effective radius
	...
	with open(writedir+'galfit_input.txt','w') as f:
		...
		f.write('C) \n')         #Sigma File (seemed to work better without, not sure why)
		f.write('D) \n')					# psf file (if wanting to put in psf, use Tiny Tim)
	...
	if result not in file_list:
		...
		with open(outdir+'fit.log','r') as f4:
			...
			#the above section is very hard-coded so might have to be changed. Couldn't find a better way
...
ascii.write(output,writedir+'fit_results_'+band+'.txt', delimiter = '\t', names=['Object','Centroid_X','X_Err','Centroid_Y','Y_Err', 'Magnitude','Mag_Err', 'R_e','R_Err', 'Sersic','b/a','ba_Err' ,'Chi^2'],overwrite = True)
