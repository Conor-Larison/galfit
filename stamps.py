...
def main():
	data = sys.argv[1]  #This is where you put in HST fits file (put directly after stamps.py in terminal ie python stamps.py 'HST FILE')
	reg = sys.argv[2]  #This is where you put in the region file (after the HST file)
	a = open(reg, 'r')
	lines = a.readlines()
	writedir = '/home/conor/Project1/'
	...
	for j in range(len(lines)):  #This is hard-coded, so you will need to look at the region file to make sure it will work
		ra.append(lines[j][6:18])
		dec.append(lines[j][20:31])
	...
	for i in range(len(obj_pix)): 	
		...
		for l in range(int(Y-width),int(Y+width)):
			...
			if l>=0 and l< ymax:
				...
				while k < int(X+width):
					if k>=0 and k<xmax:
						if np.isnan(img[l][k]) == True: #This attempts to include certain galaxies on the edge of the mosaic
							...
							row = np.append(row,img[l][k]* expt) # multiply pixel values by exptime for galfit to run properly
							k+=1
		...
		std = 1.4826 * stats.median_absolute_deviation(new_stamp) #more robust version of a std
		...
		if np.isnan(stamp[xm][ym]) == False and stamp[xm][ym] != 0:
			...
			stamp = np.flipud(stamp) #dimensions were originally messed up
